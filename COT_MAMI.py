...
class Classifier(pl.LightningModule):
  def __init__(self):
      super().__init__()
      self.MFB = MFB(512, 768, True, 256, 768, 0.1)
      ...
      self.cross1 = CrossAttention(768, 8, 768)  # Cross-attention layer
  ...
  def forward(self, x, y, e, r, c):
      ...
      fusion = self.MFB(torch.unsqueeze(y, axis=1), torch.unsqueeze(x, axis=1))
      fusion_tmp = torch.squeeze(fusion, dim=1)
      # print(fusion_tmp.shape, "FSTMPSHAPE")
      # fusion_tmp2 = self.fin_2(fusion_tmp)
      # print(fusion_tmp2.shape, "FSTMP2SHAPE")
      emf = self.MFB2(fusion_tmp, e)
      emf = torch.squeeze(emf,dim=1)
      # print(emf.shape, "EMFSHAPE")
      timr = self.MFB2(emf,r)
      timr = torch.squeeze(timr,dim=1)
      ccmi = self.MFB2(timr, c)
      ccmi = torch.squeeze(ccmi,dim=1)

      # fusion_tmp2 = self.fin_2(fusion_tmp)
      # emf = self.cross1(fusion_tmp2,e)
      # timr = self.cross1(emf,r)
      # ccmi = self.cross1(timr,c)
      # c = self.fin_3(ccmi)
      # c = self.fin_old(fusion_tmp)

      output = torch.log_softmax(ccmi, dim=1)
      #print("output shape", output.shape)
      return output,emf,timr,ccmi
